...
public class LancementEnFX extends Application {
	...
	private void draw(GraphicsContext gc,Plateau plateau){
		if(menu){
		}else{
			for (int x=0; x<tailleX; x++){
				for (int y=0; y<tailleY; y++){
					if 	  (plateau.getGrille()[y][x] instanceof Char) {
						if(plateau.getGrille()[y][x].getEquipe()==1){
					...
					else if(plateau.getGrille()[y][x] instanceof Obstacle) {
						gc.drawImage(images.get("1obstacle"),x*tailleParcelle,y*tailleParcelle+100);
					}
					else if(plateau.getGrille()[y][x] instanceof Base) {
						if(plateau.getGrille()[y][x].getEquipe()==1){
					...
					else if(plateau.getGrille()[y][x] instanceof Tireur) {
						if(plateau.getGrille()[y][x].getEquipe()==1){
					...
					else if(plateau.getGrille()[y][x] instanceof Mine) {
						if(plateau.getGrille()[y][x].getEquipe()==1 && tourJ1){
							gc.drawImage(images.get("1mine"),x*tailleParcelle,y*tailleParcelle+100);
						}
						else if(plateau.getGrille()[y][x].getEquipe()==2 && !tourJ1){
					...
					else if(plateau.getGrille()[y][x] instanceof Piegeur) {
						if(plateau.getGrille()[y][x].getEquipe()==1){
